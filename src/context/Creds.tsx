...
import { SortedCreds } from "../types";

type CredsContextType = {
  sortedCreds: SortedCreds | null;
  loadingCreds: boolean;
  reloadCreds: () => Promise<any>;
  storeCreds: (sortedCreds: any, kolpProof: any) => Promise<any>;
};

const CredsContext = createContext<CredsContextType>({
  sortedCreds: null,
  loadingCreds: true,
  reloadCreds: () => Promise.resolve(),
  storeCreds: () => Promise.resolve(),
});

function CredsProvider({ children }: { children: React.ReactNode }) {
  ...
  const [sortedCreds, setSortedCreds] = useState<SortedCreds | null>(null);
  ...
  const loadCreds = useCallback(async () => {
    ...
    try {
      const sortedCredsTemp = await getCredentials(
        holoKeyGenSigDigest,
        holoAuthSigDigest,
        false
      );
  ...
  async function storeCreds(sortedCreds: SortedCreds, kolpProof: any) {
